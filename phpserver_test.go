...
func phpServerProbeURL(pathInfo string) string {
	return "http://example.com/server-variable.php" + pathInfo
}
...
func TestPhpServer(t *testing.T) {
	t.Run("idx", func(t *testing.T) {
		...
		body1, _ := phpServerGet(t, frankenphp.PhpServers[1], phpServerProbeURL(""))
		body2, _ := phpServerGet(t, frankenphp.PhpServers[2], phpServerProbeURL(""))
	...
	t.Run("env", func(t *testing.T) {
		...
		body, _ := phpServerGet(t, server, phpServerProbeURL(""))

		assert.Contains(t, body, "[PHP_SERVER_TEST_KEY] => from_php_server")
	})

	t.Run("split_path", func(t *testing.T) {
		server := initPhpServerWithOptions(t, 1, frankenphp.WithPhpServerSplitPath([]string{".php"}))

		body, _ := phpServerGet(t, server, "http://example.com/server-globals.php/pathinfo")
	...
	t.Run("workers_by_path", func(t *testing.T) {
		initPhpServer(t, frankenphp.WithPhpServer(1,
			frankenphp.WithPhpServerRoot(testDataDir, false),
		...
		assert.Equal(t, "requests:1", body1)
		assert.Equal(t, "requests:2", body2)
	})

	t.Run("workers_with_request_matcher", func(t *testing.T) {
		initPhpServer(t, frankenphp.WithPhpServer(1,
			frankenphp.WithPhpServerRoot(testDataDir, false),
			frankenphp.WithPhpServerWorker("match", testDataDir+"dedup-match-worker.php", 1,
		...
		matched, _ := phpServerGet(t, frankenphp.PhpServers[1], "http://example.com/match/anything")
		regular, _ := phpServerGet(t, frankenphp.PhpServers[1], "http://example.com/server-globals.php")

		assert.Equal(t, "dedup-match-worker", matched)
		assert.Contains(t, regular, "SCRIPT_NAME: /server-globals.php\n")
	})
	...
	t.Run("duplicate_registration", func(t *testing.T) {
		...
		body, _ := phpServerGet(t, frankenphp.PhpServers[1], phpServerProbeURL(""))
	...
	t.Run("serve_http_validation", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, phpServerProbeURL(""), nil)
