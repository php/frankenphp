...
/* Declares Io\Poll\Handle on a handle class where the poll API exists.
 * The interface is internal only, an internal class is what it takes. */
...
static void frankenphp_task_obj_get_stream(zval *zthis, zval *return_value) {
  ...
  zend_resource *res = FRANKENPHP_TASK_DATA(zthis)->res;
  GC_ADDREF(res);
  RETURN_RES(res);
}
