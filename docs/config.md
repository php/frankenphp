...
php_server [<matcher>] {
	...
	worker { # Creates a worker specific to this server. Can be specified more than once for multiple workers.
		...
		ping <interval> <message> # Sends a periodic message to the worker via frankenphp_handle_request(). Interval can be a duration (e.g. 60s, 1m) or a cron-like keyword (minutely, hourly). Can be specified more than once.
...
Workers can receive periodic messages via the `ping` directive.
The message is passed as the first argument to the closure given to `frankenphp_handle_request()`, like [extension workers](extension-workers.md#headless-mode-sendmessage).
...
You can specify multiple `ping` lines to schedule different messages at different intervals.
